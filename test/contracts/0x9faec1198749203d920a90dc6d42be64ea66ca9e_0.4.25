...
contract SuperFan is ExpandedToken
{
		...
		uint shareArtist) public 
	{
		...
		require(shareArtist>=0 && shareArtist<=50);
	...
	function changeId(uint[] tokenId, uint64[] oldId, uint64[] newId) public onlyAdmin
	{
		require(paused == false);
		uint count = tokenId.length;
		require(count == oldId.length);
		require(count == newId.length);
		
		for(uint i = 0; i < count; i++) 
		{
			require(ownerOf(tokenId[i]) == oldId[i]);
			_transfer(oldId[i], newId[i], tokenId[i]);
		}
	}
